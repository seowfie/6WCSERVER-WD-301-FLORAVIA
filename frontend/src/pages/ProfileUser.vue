...
// 🖼️ FINAL SAFE Image Helper
function getImageUrl(path) {
  // 1. Safety Check: Return internet placeholder if path is missing
  if (!path) return "https://placehold.co/400?text=No+Image";

  const clean = String(path).trim();

  // 2. Handle Backend Uploads (Connect to Render)
  if (clean.startsWith("/uploads/")) {
    return `${import.meta.env.VITE_API_BASE_URL}${clean}`;
  }

  // 3. Handle External Links (Facebook, Google, etc.)
  if (clean.startsWith("http")) {
    return clean;
  }

  // 4. Handle Local Images (The Fix)
  // Since we moved images to the "public" folder, we just return the path string!
  // Example: If path is "profile.png", this returns "/profile.png"
...
const saveAddress = async () => {
  ...
  try {
    const res = await api.patch(
      `/api/users/${user.value._id}/address`,
